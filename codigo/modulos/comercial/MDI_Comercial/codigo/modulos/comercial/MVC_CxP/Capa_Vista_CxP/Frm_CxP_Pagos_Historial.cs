...
namespace Capa_Vista_CxP
{
    public partial class Frm_CxP_Pagos_Historial : Form
    {
        ...
        private void CargarHistorial()
        {
            ...
            // Los montos deben ser DECIMAL NORMAL, no decimal?
            decimal montoMin = 0m;
            decimal montoMax = 0m;

            if (nudMontoDesde.Value > 0)
                montoMin = nudMontoDesde.Value;

            if (nudMontoHasta.Value > 0)
                montoMax = nudMontoHasta.Value;

            // Orden
            string ordenar = "FechaDesc";
            ...
            // 👉 ESTA ES LA LLAMADA CORRECTA (5 parámetros)
            DataTable dt = _mdlPagos.Pagos_Historial(desde, hasta, montoMin, montoMax, ordenar);
