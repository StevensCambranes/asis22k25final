...
namespace Capa_Modelo_CxP
{
    public class Cls_Sentencias_CxP_Pagos
    {
        ...
        // =====================================================
        public DataTable Pagos_Historial(
            DateTime fechaDesde,
            DateTime fechaHasta,
            decimal montoMin,
            decimal montoMax,
            ...
            try
            {
                string sql = @"
                    ...
                    INNER JOIN Tbl_Usuario u
                        ON u.Cmp_Id_Usuario = p.Cmp_Id_Usuario
                    WHERE p.Cmp_Fecha_Pago BETWEEN ? AND ?
                      AND p.Cmp_Total_Pago BETWEEN ? AND ?
                ";

                // Orden dinámico según combo
                switch (ordenar)
                {
                    case "Monto más bajo":
                ...
                {
                    cmd.Parameters.AddWithValue("@p1", fechaDesde);
                    cmd.Parameters.AddWithValue("@p2", fechaHasta);
                    cmd.Parameters.AddWithValue("@p3", montoMin);
                    cmd.Parameters.AddWithValue("@p4", montoMax);
